...
<template>
  <div class="basket-container">
    <ul v-if="basket.length > 0" class="basket-list">
      <li v-for="item in basket" :key="item.product._id + item.orderId" class="basket-item">
        <h3>{{ item.product.name.full }}</h3>
        <p>Цена: {{ item.product.price * item.quantity }} ₸</p>
        <p>
          Количество:
          <input type="number" :value="item.quantity" @input="onQuantityChange(item.product._id, $event)" min="1"
            :disabled="item.isPurchased" />
        </p>
        <div class="basket-list-button-wrapper">
          <MainButton @click="deleteItem(item.product._id, item.orderId)" :disabled="item.isPurchased">
            Удалить
          </MainButton>
        </div>
      </li>
      ...
      <MainButton color="blue" :disabled="totalAmount === 0" @click="checkoutBasket2">
        Оплатить</MainButton>
    </ul>
